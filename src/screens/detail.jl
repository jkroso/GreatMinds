function render_detail(m::GreatMindsApp, area::Rect, buf::Buffer)
    ...
    elseif !isempty(m.detail_error)
        reply_block = Block(title="Error", border_style=tstyle(:error))
        reply_inner = render(reply_block, rects[3], buf)
        render(Paragraph(m.detail_error; wrap=word_wrap, style=tstyle(:error)),
               margin(reply_inner; left=1, right=1), buf)
